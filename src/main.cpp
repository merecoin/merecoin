...
#if defined(NDEBUG)
# error "Merecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3c1005c995d5226eace00d173fb4429e782f88ef2709bee33587883d759d390c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Merecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Merecoin Signed Message:\n";
...
{
    // Merecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 7700000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 7700000); // Merecoin: 7700k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Merecoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Merecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Merecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Merecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x915c20097610e96d7c5e0231abe677680aa4ce0b78cc2b763bc0734d9c68dcc1");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 01/Apr/2025 Merecoin is traded on the NYSE at $100k per coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043c15fe7ff6c40091bb571629416d92c8284b9a0cc5ab2f07a36da2689330d883222d724652ba44c8683aa5897bac0919d57bf2916dea658e4670f770c82a0ce5") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1522422352;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 271727;

        if (fTestNet)
        {
            block.nTime    = 1522450999;
            block.nNonce   = 212322;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x0d03a83d737e4c79b7a90a10d0ea51e2c3616ba47b70d481e606225aac6caaca"));
		//GENESIS GENERATE BEGIN
		
		//GENESIS GENERATE END
        block.print();
...
unsigned char pchMessageStart[4] = { 0x11, 0x22, 0x31, 0xaa }; // Merecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MerecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MerecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MerecoinMiner(CWallet *pwallet)
{
    printf("MerecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("merecoin-miner");
    ...
    try { loop {
        ...
        printf("Running MerecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MerecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MerecoinMiner, pwallet));
}
